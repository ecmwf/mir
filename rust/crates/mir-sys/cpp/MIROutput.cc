...
const T& as(const std::unique_ptr<mir::output::MIROutput>& output, const char* what) {
    ...
    if (cast == nullptr) {
        throw eckit::SeriousBug(std::string("MIROutputWrapper::") + what);
    }
    return *cast;
}
...
std::unique_ptr<MIROutputWrapper> MIROutputWrapper::to_callback(rust::Box<OutputBox> output) {
    auto wrapper     = std::make_unique<MIROutputWrapper>();
    wrapper->output_ = std::make_unique<CallbackOutput>(std::move(output));
    return wrapper;
}

std::unique_ptr<MIROutputWrapper> MIROutputWrapper::to_grib_file(rust::Str path, bool append) {
    auto wrapper     = std::make_unique<MIROutputWrapper>();
    wrapper->output_ = std::make_unique<mir::output::GribFileOutput>(eckit::PathName(std::string(path)), append);
    return wrapper;
}

std::unique_ptr<MIROutputWrapper> MIROutputWrapper::to_grib_memory(size_t capacity) {
    auto wrapper      = std::make_unique<MIROutputWrapper>();
...
std::unique_ptr<MIROutputWrapper> MIROutputWrapper::to_resizable() {
    auto wrapper     = std::make_unique<MIROutputWrapper>();
    wrapper->output_ = std::make_unique<mir::output::ResizableOutput>(wrapper->values_, wrapper->metadata_);
    return wrapper;
}

std::unique_ptr<MIROutputWrapper> MIROutputWrapper::to_empty() {
    auto wrapper     = std::make_unique<MIROutputWrapper>();
