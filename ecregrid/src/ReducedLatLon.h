...
// -- Methods
	virtual GridContext* getGridContext() const { return Reduced::getGridContext(); }
...
// -- Overridden methods
	...
	bool reduced()                    const { return true; }

	size_t  getGridDefinition(vector<long>& v) const;

	bool  areValidIncrements() const;
	virtual Area  setGlobalArea() const;
    virtual Area  setGlobalArea(double west)  const { return LatLon::setGlobalArea(west); }

	bool  isGlobalWestEast()         const ;

	string   composedName()  const { return "reduced_ll"; }
	...
	virtual bool sameAs(const RotatedRegularLatLon& other)   const  { return LatLon::sameAs(other); }
