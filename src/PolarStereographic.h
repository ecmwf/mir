...
// Overriden methods
    int westEastNumberOfPoints()      const {return xDirectionNumberOfPoints_; }
    int northSouthNumberOfPoints()    const {return yDirectionNumberOfPoints_; }
    double  northSouthIncrement()     const {return float(dyInMetres_); }
	double  westEastIncrement()       const {return float(dxInMetres_); }
...
// Overriden methods
	...
	string   composedName()  const { return "polar_stereographic"; }

	virtual bool sameAs(const RegularLatLon& other)   const  { return Projection::sameAs(other); }
	virtual bool sameAs(const RegularLatLonCellCentered& other)   const  { return Projection::sameAs(other); }
	virtual bool sameAs(const ReducedLatLon& other)   const  { return Projection::sameAs(other); }
	virtual bool sameAs(const RotatedRegularLatLon& other)   const  { return Projection::sameAs(other); }
	virtual bool sameAs(const ReducedGaussian& other) const  { return Projection::sameAs(other); }
	virtual bool sameAs(const RegularGaussian& other) const  { return Projection::sameAs(other); }
	virtual bool sameAs(const PseudoRegularGaussian& other) const  { return Projection::sameAs(other); }
	virtual bool sameAs(const  PolarStereographic& other) const  ;
	virtual bool sameAs(const ListOfPoints& other)    const  { return Projection::sameAs(other); }
	bool equals(const Grid& other) const { return other.sameAs(*this); }
