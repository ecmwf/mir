...
const double R          = R_POL / R_EQ;


Point2 forward_projection(const PointLatLon& ll) {
    auto mv  = std::numeric_limits<double>::infinity();
    auto lat = ll.lat();
    auto lon = ll.lon();
    ...
    double c_lat = std::atan(R * R * std::tan(latr));  //< geocentric latitude
    double re    = R_POL / std::sqrt(1. - (1. - R * R) * std::cos(c_lat) * std::cos(c_lat));
    ...
    auto dot = r1 * rl * std::cos(c_lat) * std::cos(lonr) - r2 * r2 - r3 * r3 / (R * R);

    return {dot <= 0 ? mv : std::atan(-r2 / r1), dot <= 0 ? mv : std::asin(-r3 / rn)};
}
...
PointLatLon inverse_projection(const Point2& xy) {
    ...
    double f2 = std::cos(y) * std::cos(y) + std::sin(y) * std::sin(y) / (R * R);
    ...
    auto lat = util::radian_to_degree(std::atan(s3 / (sxy * R * R)));
    return {lat, lon};
}


template <typename EXTERNAL_T = double, typename INTERNAL_T = EXTERNAL_T>
...
void interpret(const param::MIRParametrisation& param, const data::MIRField& field) {
    ...
    auto Nr = get(param, "NrInRadiusOfEarth") * (get<long>(param, "edition") == 1 ? 1e-6 : 1.);
    ASSERT(Nr > 1.);
    auto height = (Nr - 1.) * R_EQ;

    auto dx   = get<double, long>(param, "dx");
    auto dy   = get<double, long>(param, "dy");
    auto coff = get<double, long>(param, "XpInGridLengths");
    auto loff = get<double, long>(param, "YpInGridLengths");
    ...
    auto slat = get(param, "latitudeOfSubSatellitePointInDegrees");
    auto slon = get(param, "longitudeOfSubSatellitePointInDegrees");
    ...
    auto scale       = static_cast<double>(std::numeric_limits<uint16_t>::max());  // 65536.
    double cfac      = -scale * double(dx) / angularSize;
    double lfac      = -scale * double(dy) / angularSize;
    ...
    for (size_t i = 1900; i < Ni; ++i) {
        for (size_t j = 1000; j < Nj; ++j, ++matrix_it) {
            auto lat = south - j * dlat;
            auto lon = east - i * dlon;
            ...
            auto c = static_cast<int>(std::lround(coff + xy1.x() * std::pow(2., -16) * cfac));
            auto l = static_cast<int>(std::lround(loff + xy1.y() * std::pow(2., -16) * lfac));
