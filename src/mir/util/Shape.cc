...
Shape::Shape(const param::MIRParametrisation& param) {
    ...
    param.get(isOblate ? "earthMajorAxis" : "radius", a = Earth::radius());
    param.get(isOblate ? "earthMinorAxis" : "radius", b = Earth::radius());
}


Shape::Shape(const Spec& spec) : provided(false) {
    if (spec.has("radius")) {
        code = 1L;
