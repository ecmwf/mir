...
MIRCompare::MIRCompare(int argc, char** argv) :
    MIRTool(argc, argv),
    user_absolute_(1e-9),
    user_relative_(1e-9),
    user_percent_(1e-9),
    ...
    options_.push_back(new SimpleOption< double >("absolute", "Maximum absolute error"));
    ...
    options_.push_back(new SimpleOption< double >("packing",  "Comparing to packing error, with provided factor"));
    options_.push_back(new SimpleOption< bool   >("ulps",     "Comparing with ULPS (?)"));
    options_.push_back(new SimpleOption< bool   >("l2norm",   "Compute L2 norm between 2 fields"));
}
