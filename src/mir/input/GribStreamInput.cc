...
bool GribStreamInput::next() {
    ...
    for (size_t i = 0; i < advance; i++) {
        ...
        if (e == GRIB_BUFFER_TOO_SMALL) {
            eckit::Log::trace<MIR>() << "GribStreamInput::next() message is " << len << " bytes (" << eckit::Bytes(len) << ")" << eckit::newl;
    ...
    if (e == GRIB_BUFFER_TOO_SMALL) {
        eckit::Log::trace<MIR>() << "GribStreamInput::next() message is " << len << " bytes (" << eckit::Bytes(len) << ")" << eckit::newl;
        eckit::Log::trace<MIR>() << "Buffer size is " << buffer_.size() << " bytes (" << eckit::Bytes(buffer_.size()) << "), rerun with:" << eckit::newl;
        eckit::Log::trace<MIR>() << "env MIR_GRIB_INPUT_BUFFER_SIZE=" << len << eckit::newl;
        GRIB_ERROR(e, "wmo_read_any_from_stream");
    }
