...
void ConservativeFiniteElement::assemble(util::MIRStatistics& statistics, WeightMatrix& W, const repres::Representation& in,
    ...
    auto Min = atlas::array::make_view<double, 1, atlas::array::Intent::ReadOnly>(
        inputMethod_->atlasMesh(statistics, in).nodes().field("node_lumped_mass_matrix"));
    ...
    auto Mout = atlas::array::make_view<double, 1, atlas::array::Intent::ReadOnly>(
        outputMethod_->atlasMesh(statistics, out).nodes().field("node_lumped_mass_matrix"));
