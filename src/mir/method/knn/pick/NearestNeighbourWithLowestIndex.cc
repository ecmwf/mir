...
void NearestNeighbourWithLowestIndex::pick(const search::PointSearch& tree,  const PointXYZ& point,
    ...
    size_t c        = 0;
    const double d2 = PointXYZ::distance2(point, neighbours.front().point().to_xyz());

    for (size_t j = 1; j < neighbours.size(); ++j) {
        if (eckit::types::is_strictly_greater(PointXYZ::distance2(point, neighbours[j].point().to_xyz()), d2)) {
            break;
        }
