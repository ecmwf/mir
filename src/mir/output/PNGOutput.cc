...
PNGOutput::PNGOutput(std::string path) : path_(std::move(path)), once_(true) {}

bool PNGOutput::once() {
    if (!once_) {
        std::ostringstream oss;
        oss << "PNGOutput: attempt to write more than once to " << *this;
...
size_t PNGOutput::save(const param::MIRParametrisation& param, context::Context& ctx) {
    ...
    // setup encoder
    std::unique_ptr<const PNGEncoder> encoder(
        PNGEncoderFactory::build(param, values, field.hasMissing(), field.missingValue()));
    ASSERT(encoder);
    ...
    call_nonzero(fp = std::fopen(path_.c_str(), "wb"), "write to '" + path_ + "'");
...
const PNGOutput::PNGEncoder* PNGEncoderFactory::build(const param::MIRParametrisation& param,
                                                      const MIRValuesVector& values, bool hasMissing,
                                                      double missingValue) {
    pthread_once(&once, init);
    eckit::AutoLock<eckit::Mutex> lock(local_mutex);

    std::string name;
    param.get("png-output-encoder", name = "gray@8bpp");
    ...
    return (*j).second->make(param, values, hasMissing, missingValue);
}
...
template <int N_CHANNELS, int N_BYTES_PER_CHANNEL, typename UINT_T>
struct PNGEncoderT : PNGOutput::PNGEncoder {

    PNGEncoderT(const param::MIRParametrisation& param, const MIRValuesVector& values, bool hasMissing,
                double missingValue) :
        ...
        if (param.get("png-output-minmax", minmax)) {
            ...
            bool first = true;
            min_ = max_ = std::numeric_limits<double>::signaling_NaN();
            for (auto& v : values) {
        ...
        maxEncode_ = (UINT_T(1) << N_CHANNELS * N_BYTES_PER_CHANNEL * 8) - 1;
    ...
    void encode(png_bytep& p, const double& value) const {
        if (hasMissing_ && value == missingValue_) {
            ...
            for (int i = 0; i < (N_CHANNELS + 1) * N_BYTES_PER_CHANNEL; ++i) {
                *(p++) = 0x00;
            }
            return;
        }
        ...
        UINT_T mask(UINT_T(0xFF) << 8 * (N_CHANNELS * N_BYTES_PER_CHANNEL - 1));

        for (int byte = N_CHANNELS * N_BYTES_PER_CHANNEL - 1; byte >= 0; --byte, mask = (mask >> 8)) {
    ...
    int color_type() const {
        return N_CHANNELS == 1
                   ? (hasMissing_ ? PNG_COLOR_TYPE_GRAY_ALPHA : PNG_COLOR_TYPE_GRAY)
                   : N_CHANNELS == 3 ? (hasMissing_ ? PNG_COLOR_TYPE_RGB_ALPHA : PNG_COLOR_TYPE_RGB) : NOTIMP;
    }
...
static PNGEncoderBuilder<PNGEncoderT<1, 1, uint_fast8_t>> encoder1("gray/8bpp");
static PNGEncoderBuilder<PNGEncoderT<1, 2, uint_fast16_t>> encoder2("gray/16bpp");
static PNGEncoderBuilder<PNGEncoderT<3, 1, uint_fast32_t>> encoder3("rgb/8bpp");
static PNGEncoderBuilder<PNGEncoderT<3, 2, uint_fast64_t>> encoder4("rgb/16bpp");
