/*
 * (C) Copyright 1996-2015 ECMWF.
 *
 * This software is licensed under the terms of the Apache Licence Version 2.0
 * which can be obtained at http://www.apache.org/licenses/LICENSE-2.0.
 * In applying this licence, ECMWF does not waive the privileges and immunities
 * granted to it by virtue of its status as an intergovernmental organisation nor
 * does it submit to any jurisdiction.
 */

/// @author Baudouin Raoult
/// @author Pedro Maciel
/// @date Apr 2015

#include "mir/output/ParamOutput.h"

#include <iostream>

#include "eckit/exception/Exceptions.h"

#include "mir/param/RuntimeParametrisation.h"


namespace mir {
namespace output {

// This class change the param id of the field (for TP = CP + LSP)

ParamOutput::ParamOutput(MIROutput &output):
    output_(output) {
}


ParamOutput::~ParamOutput() {
}


size_t ParamOutput::copy(const param::MIRParametrisation &param, input::MIRInput &input) {
    NOTIMP;
}

size_t ParamOutput::save(const param::MIRParametrisation &param, input::MIRInput &input, data::MIRField &field) {
    param::RuntimeParametrisation runtime(param);
    runtime.set("param-id", param_);
    return output_.save(runtime, input, field);
}


}  // namespace output
}  // namespace mir

