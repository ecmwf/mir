...
int main(int argc, char** argv) {
	...
	err = ecregrid_set_increments(f,1.0,1.0);
	if(err != 0){
		printf("ERROR: unable to set field description \n");
		return 1;
	}

	/* err = ecregrid_set_truncation(f,399); */
 /* rotation */
	err = ecregrid_set_south_pole(f,-30.0,10.0);
/*
*/
	err = ecregrid_set_area(f,80.0,-30.0,50.0,40.0);
	err = ecregrid_set_area(f,80.0,30.0,50.0,50.0);
	err = ecregrid_set_area(f,80.0,-30.0,50.0,-10.0);
	err = ecregrid_set_area(f,80.0,0,50.0,360.0);
	if(err != 0){
		printf("ERROR: unable to set field description \n");
		return 1;
	}
